...
if [[ -n $warn ]] && [[ -z $crit ]]; then echo "UNKNOWN: Both warning and critical thresholds must be set"; exit $STATE_UNKNOWN; fi
if [[ -z $warn ]] && [[ -n $crit ]]; then echo "UNKNOWN: Both warning and critical thresholds must be set"; exit $STATE_UNKNOWN; fi
if [[ $warn -gt $crit ]]; then echo "UNKNOWN: Warning threshold cannot be greater than critical"; exit $STATE_UNKNOWN; fi
#########################################################################
...
then
  ...
  else echo "OK: All zfs pools ok (${POOLS[*]}) | ${perfdata[*]}"; exit 0
  fi
...
## Check single pool
  ...
  if [[ -n $(echo "${CAPACITY}" | egrep -q 'no such pool$') ]]; then
    echo "CRITICAL: zpool $pool does not exist"; exit $STATE_CRITICAL
  fi
  HEALTH=$(zpool list -Ho health "$pool")
  if [ $? -ne 0 ]; then
    echo "UNKNOWN: zpool query failed"; exit $STATE_UNKNOWN
  ...
  then
    # Check with thresholds
    if [ "$HEALTH" != "ONLINE" ]; then echo "CRITICAL: zfs pool $pool health is $HEALTH | $pool=${CAPACITY}%"; exit ${STATE_CRITICAL}
    elif [[ $CAPACITY -ge $crit ]]; then echo "CRITICAL: zfs pool $pool usage is CRITICAL (${CAPACITY}%) | $pool=${CAPACITY}%"; exit ${STATE_CRITICAL}
    ...
    else echo "OK: All zfs pools ok ($pool) | $pool=${CAPACITY}%"; exit ${STATE_OK}
    fi
  else
    # Check without thresholds
    if [ "$HEALTH" != "ONLINE" ]
    then echo "CRITICAL: zfs pool $pool health is $HEALTH | $pool=${CAPACITY}%"; exit ${STATE_CRITICAL}
    else echo "OK: All zfs pools ok ($pool) | $pool=${CAPACITY}%"; exit ${STATE_OK}
    fi
  fi
